...
import ctn.imaginarycraft.common.item.weapon.BulletItem;
import ctn.imaginarycraft.common.item.weapon.RangedWeaponItem;
...
public abstract class AbstractGunItem extends RangedWeaponItem {
   ...
   * 生物开火逻辑。
   */
  abstract public void shoot(Level level, LivingEntity livingEntity, 
                             ItemStack stack, Projectile bullet);
  ...
  public Projectile getBulletProjectile(Level level, LivingEntity shooter, ItemStack stack) {
    if (!this.isConsumingBullets(shooter, stack)) {
      return this.getDefaultBullet().get().asProjectile(level, shooter.position(), stack, shooter.getDirection());
    }
