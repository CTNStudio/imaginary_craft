...
public class ServerTentacle {
  ...
  public ServerTentacle(
    ...
    for (int i = 0; i < segmentCount; i++) {
      ...
      TentacleSegment segment = new TentacleSegment(length, maxPitch, minPitch, maxYaw, minYaw);
  ...
  public void moveSegmentTo(int segmentIndex, Vector3f targetPosition, int iterations) {
    ...
    for (int iter = 0; iter < iterations; iter++) {
      ...
      for (int i = segmentIndex; i > 0; i--) {
        TentacleSegment prev = current.prev;

        if (prev == null) break;

        Vector3f toCurrent = new Vector3f(current.getWorldPosition()).sub(prev.getWorldPosition());
        toCurrent.normalize();

        Vector3f toTarget = new Vector3f(targetPosition).sub(prev.getWorldPosition());
        toTarget.normalize();
        ...
        if (angle > 0.001f) {
          ...
          Quaternionf prevRotInv = new Quaternionf(prev.rotation).invert();
          Vector3f localAxis = prevRotInv.transform(new Vector3f(axis));

          float deltaPitch = (float) Math.asin(localAxis.y * Math.sin(angle));
          float deltaYaw = (float) Math.atan2(localAxis.x, localAxis.z);

          prev.addSwingAngle(deltaPitch * 0.1f, deltaYaw * 0.1f);
        }
  ...
  public static class TentacleSegment {
    ...
    // 链式结构引用
    TentacleSegment prev;  // 前一节
    TentacleSegment next;  // 下一节
